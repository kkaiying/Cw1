...
public class PathFindingService {
    ...
    public List<LngLat> findPath(LngLat start, LngLat goal, RestrictedAreaDTO[] restrictedAreas) {
        // Check if goal is inside any restricted area
        for (RestrictedAreaDTO area : restrictedAreas) {
            if (calculationService.inRegion(goal, area.vertices)) {
                return Collections.emptyList(); // Goal unreachable
            }
        }
    ...
    public String roundedKey(LngLat pos) {
        return String.format("%.5f, %.5f", pos.lng, pos.lat);
    }

    public List<LngLat> reconstructPath(Node node) {
    ...
    public boolean pathCrossesRestrictedArea(LngLat from, LngLat to, RestrictedAreaDTO[] restrictedAreas) {
        for (RestrictedAreaDTO area : restrictedAreas) {
